...
public class GUIBasedMultiExchangeSenderReceiver {
    ...
    private void setUpSocket(){
        ...
        try {
            ...
            // Create a socket with the hard-coded
            ...
            showMessageDialog(configPanel, "Socket could not be bound to port " + senderPort + ". Please choose another " +
    ...
    private void setRecipientDetails(){
        try {
            ...
            // Validate to confirm the port is a legal one
            if(port < 1 || port > 65535){
                throw new NumberFormatException("Illegal port value supplied");
            }

            // Once it has been validated, save entered port as destination port
            receiverPort = port;
    ...
    private void sendMessage(){
        ...
        try {
            // Convert the message to be sent into a packet
            byte [] payload = enteredText.getBytes();
            ...
            // Convert the received response from the packet into a String and add it to the message exchange text area
            String response = new String(reply.getData(), 0, reply.getLength());
            messageExchangeTextArea.append(ipAddress + "> " + response + "\n");
        }catch(IOException e){
